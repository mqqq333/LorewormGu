...
def init_model(llm_config):
     tokenizer = AutoTokenizer.from_pretrained('/home/u3210104896/LorewormGu')  # 加载分词器
     model = LorewormGu(llm_config).to(args.device)  # 初始化模型并移动到设备
...
if __name__=='__main__':
     ...
     parser.add_argument("--wandb_project", type=str, default="LorewormGu-Pretrain")  # wandb项目名称
     ...
     # 设置wandb运行名称
     args.wandb_run_name = f"LorewormGu-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
